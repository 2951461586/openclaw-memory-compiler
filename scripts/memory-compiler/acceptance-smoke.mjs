...
import { buildSourceActionPlan } from './lib/runtime-bridge-core.mjs';
...
record('runtime-bridge-source-action-plan-reasons-normalized', inferredPreciseAnchored.ok === true && ['source-summary-expand', 'source-summary-expand-scoped', 'source-summary-discover-scoped', 'source-summary-search'].includes(String(inferredPreciseAnchored.sourceActionPlan?.primary?.reason || '')) && !(JSON.stringify(inferredPreciseAnchored.sourceActionPlan || {}).includes('memory_recall')) && !(String(inferredPreciseAnchored.prependSystemContext || '').includes('Primary tool call:')) && !(String(inferredPreciseAnchored.prependSystemContext || '').includes('optional commentary')), { sourceActionPlan: inferredPreciseAnchored.sourceActionPlan, prependSystemContext: inferredPreciseAnchored.prependSystemContext });
...
const queryOnlyPlan = buildSourceActionPlan({
  scene: 'precise',
  recallPlan: {
    strategy: 'verify-selected-sources-before-answer',
    reason: 'source-first-verification',
    queryTerms: ['real', 'import', 'inline', 'lcm', 'summaries', 'adapter', 'entry'],
    candidateRefs: []
  },
  reviewTriage: { topItems: [] }
}, { prompt: '精确回答：real import should accept inline LCM summaries via adapter entry 这条主线落在哪个 thread？' });
record('runtime-query-only-cross-conversation-discovery-visible', queryOnlyPlan?.primary?.tool === 'lcm_expand_query' && queryOnlyPlan?.primary?.reason === 'source-summary-discover-scoped' && Number.isFinite(Number(queryOnlyPlan?.primary?.params?.conversationId)) && Array.isArray(queryOnlyPlan?.primary?.params?.summaryIds) && queryOnlyPlan?.primary?.params?.summaryIds?.length >= 1, { queryOnlyPlan });
record('runtime-query-only-fallback-naming-clean', !(JSON.stringify(queryOnlyPlan || {}).includes('memory_recall')) && ['source-search-fallback', 'source-memory-cross-check'].includes(String(queryOnlyPlan?.fallbacks?.[0]?.reason || '')), { queryOnlyPlan });
